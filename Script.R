...
# Thankx to Adam Check for the great YouTube videos: https://www.youtube.com/watch?v=dBNy_A6Zpcc
######################################################
...
data <- read.delim ("data/Passengers_Cars_Registrations_Italy_1990_2021.csv", sep = ";") # Passenger Cars Registrations in Italy from 1990 - 2021

Y <- ts(data[,2], start = c(1990,1), frequency = 12) # declare a new variable Y as Time Series (just the second column)
Y
...
fit <- snaive(DY)   # Residual SD 28006.2207
print(summary(fit))   
checkresiduals(fit)

# as alternative to checkresiduals 
res <- residuals (fit)
...
# retreiving 2022 h1 registrations data https://www.acea.auto/pc-registrations/passenger-car-registrations-14-0-first-half-of-2022-15-4-in-june/
italy_2022_h1 <- c(107814, 110869, 119497, 97339, 121299, 127209)

italy_2022_h1 <- ts(italy_2022_h1, start = c(2022,1), end = c(2022,6), frequency = 12)

fcst <- forecast(fit_ets,h=6) # 2022 Jan, Feb, Mar, Apr, May, Jun

accuracy (fcst,x = italy_2022_h1)

plot(fcst)
lines(italy_2022_h1)

#autoplot(fcst, include = 6)
tail(fcst)
